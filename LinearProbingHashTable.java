...
public class LinearProbingHashTable<K, V> {
    ...
    public void put(K key, V value) {
        int pos = hash(key);
        int currentProbes = 0;
        while (table[pos].isOccupied && !table[pos].isDeleted) {
            if (Objects.equals(table[pos].key, key)) {
                table[pos].value = value; return;
            }
            pos = (pos + 1) % capacity;
            currentProbes++;
        }
        table[pos].key = key; table[pos].value = value;
        table[pos].isOccupied = true; table[pos].isDeleted = false;
        size++;
